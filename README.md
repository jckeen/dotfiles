...
| **Antigravity permissions** | `antigravity/permissions.json` | Curated allow/ask/deny rules plus `toolPermission: proceed-in-sandbox` and `enableTerminalSandbox: true`. `setup.sh` merges it into the machine-local `~/.gemini/antigravity-cli/settings.json` via `claude/scripts/agy-apply-permissions.py` (never symlinked: that file also holds `trustedWorkspaces` and prompt-saved grants). `check-antigravity.sh` warns on drift; `agy-apply-permissions.py prune` resets accreted grants to the baseline after a backup. A `retired` array lists grants the baseline has withdrawn: because the merge is additive, `apply` deletes each one from the live `allow` list and `check` fails while any remain, so revoking a grant reaches machines that already installed it. `ask` and `deny` are never touched, so an operator-added restriction is never retired away |
...
| `cx [project]` | Same ergonomics for Codex; reuses a listening shared server when Remote Control is enabled and otherwise launches locally; daemon startup and repair are explicit maintenance actions |
