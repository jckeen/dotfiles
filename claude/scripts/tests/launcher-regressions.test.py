...
class LauncherRegressions(unittest.TestCase):
    ...
    def test_zsh_sources_and_uses_actual_alias_path(self):
        ...
        result = self.shell(
            'set -u; source "$ALIASES"; '
            'test "$_BASH_ALIASES_PATH" = "$ALIASES"; '
            "whence cc cx cct agy >/dev/null",
            "zsh",
        )
    ...
    def memory_fixture(self, name):
        ...
        def git(*args, cwd=repo):
            return subprocess.check_output(
                ["git", "-C", str(cwd), *args], env=env, text=True, stderr=subprocess.STDOUT
            )
        ...
        for launch_shell in ("bash", "zsh"):
            ...
            for state in ("dirty", "pending"):
                with self.subTest(shell=launch_shell, state=state):
                    repo, remote, note, env, git = self.memory_fixture(
                        f"memory-{launch_shell}-{state}"
                    )
                    ...
                    self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
                    self.assertEqual(
                        git("show", "main:project/memory/note.md", cwd=remote),
                        "safe memory update\n",
                    )
                    self.assertEqual(
                        git("show", "main:settings.json", cwd=remote), "initial settings\n"
                    )
                    self.assertEqual(
                        (repo / "settings.json").read_text(), "private operator settings\n"
                    )
                    self.assertEqual(git("diff", "--cached", "--name-only"), "")
                    self.assertEqual(
                        git("status", "--porcelain", "--", "project/memory/note.md"), ""
                    )
            ...
            for state in ("dirty", "pending"):
                for kind in ("content", "path"):
                    with self.subTest(shell=launch_shell, state=state, kind=kind):
                        repo, remote, note, env, git = self.memory_fixture(
                            f"memory-{launch_shell}-{state}-{kind}"
                        )
                        remote_before = git("rev-parse", "main", cwd=remote)
                        target = (
                            note if kind == "content" else repo / "project/memory/auth/session.json"
                        )
                        target.parent.mkdir(parents=True, exist_ok=True)
                        payload = (
                            "SYNTHETIC_MEMORY_SECRET\n" if kind == "content" else "private state\n"
                        )
                        ...
                        reason = (
                            ...
                            if state == "dirty"
                                ...
                                "SECRET-LIKE CONTENT"
                                if kind == "content"
                                else "includes non-memory paths"
                            )
                        )
...
""")
        self.assertEqual(self.aliases.stat().st_mtime_ns // 1_000_000_000, stamp // 1_000_000_000)
...
""")
    ...
    def test_dangling_git_symlink_is_reported_and_preserved(self):
        result = self.shell(
            'mkdir "$FIXTURE/dev"; ln -s absent "$FIXTURE/dev/.git"; '
...
""")
    ...
    def cct_fixture(self, fixture, project_name="project"):
        ...
        subprocess.run(["git", "init", "-q", "-b", "main", str(project)], env=self.env, check=True)
        startup = r"""
source "$ALIASES"
_dev_dir() { printf '%s\n' "$HOME/dev"; }
_check_critical_symlinks() { :; }
...
"""
                ...
                result = subprocess.run(
                    [launch_shell, "-c", capture, shell_name, *arguments],
                ...
                command = [
                    os.fsdecode(arg) for arg in args[args.index(os.fsencode(launch_shell)) :]
                ]
                ...
                self.assertEqual(
                    (fixture / "received").read_bytes().split(b"\0")[:-1],
                    [
                        b"--remote-control",
                        b"--chrome",
            ...
            for named in (True, False):
                for case in ("bare", "trailing", "literal"):
                    with self.subTest(shell=shell_name, named=named, case=case):
                        ...
                        project = self.cct_fixture(
                            fixture, "project;" if case == "literal" else "project"
                        )
...
"""
                        try:
                            result = subprocess.run(
                                [launch_shell, "-c", script, shell_name, *arguments],
                                cwd=fixture if named else project,
                                env=env,
                                text=True,
                                capture_output=True,
                                timeout=10,
    ...
    def test_setup_audit_never_invokes_source_checkout_private_bootstrap(self):
        ...
        self.assertFalse(
            (self.root / "bootstrap-invoked").exists(),
    ...
    def test_tmux_link_is_audited_and_repaired(self):
        ...
        result = subprocess.run(
            ["bash", str(setup_repo / "setup.sh"), "--check"],
            cwd=setup_repo,
            env=setup_env,
            text=True,
            capture_output=True,
        ...
        for target in (None, self.root / "missing", self.aliases):
            ...
            subprocess.run(
                ["bash", str(setup_repo / "setup.sh"), "--repair"],
                cwd=setup_repo,
                env=setup_env,
                capture_output=True,
                text=True,
