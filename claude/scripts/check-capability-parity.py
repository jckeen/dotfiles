...
def relative_path(value):
    ...
    path = Path(value)
    require(
        not path.is_absolute() and ".." not in path.parts, "path must stay relative to its root"
    )
    return path
...
def validate_probe(probe, public):
    ...
    require(
        isinstance(kind, str) and kind in allowed,
        "probe kind must be a read-only file/configuration check or live executable lookup",
    )
    if kind == "executable":
        require(
            isinstance(probe.get("name"), str) and probe["name"].strip(), "executable name missing"
        )
        return
    relative_path(probe.get("path"))
    if kind == "text":
        require(
            isinstance(probe.get("contains"), str) and probe["contains"].strip(),
            "text probe needs contains",
        )
    if kind == "json-key":
        key = probe.get("key")
        require(
            isinstance(key, list) and key and all(isinstance(k, str) and k for k in key),
...
def check_row(root, row):
    ...
    for field, allowed in (
        ("kind", KINDS),
        ("scope", {"required", "optional", "project"}),
        ("owner", {"public", "private"}),
        ("status", {"supported", "advisory", "unsupported"}),
    ):
        require(
            isinstance(row.get(field), str) and row[field] in allowed, field + " missing or invalid"
        )
    ...
    require(
        unsupported == (row["status"] == "unsupported"), "unsupported provider/status must agree"
    )
    for name in ("probe", "live_probe"):
        require(isinstance(row.get(name), list), name + " missing or invalid")
        require(
            bool(row[name]) != unsupported,
            name + " must be populated for supported/advisory providers only",
        )
    if unsupported:
...
def check_contract(root, manifest):
    require(
        isinstance(manifest, dict) and manifest.get("version") == 1,
        "unsupported capability contract version",
    )
    capabilities = manifest.get("capabilities")
    require(isinstance(capabilities, dict), "capabilities must be an object")
    errors = [
        cap + ": capability disposition missing" for cap in CAPABILITIES if cap not in capabilities
    ]
    for cap, runtimes in capabilities.items():
        ...
        for runtime in RUNTIMES:
            ...
            try:
                ...
                reason = (
                    "unreadable provider configuration"
...
def live_report(root, home, manifest):
    ...
    for location, sources in (
        ...
        if folder.is_dir():
            installed.update(
                str(Path(location) / child.name) for child in folder.iterdir() if child.is_dir()
            )
    return dict(
        live=rows,
        local_additions=sorted(installed - modeled),
...
def main():
    ...
    parser.add_argument(
        "--live-home", type=Path, help="Report only; read this home without running providers"
    )
    args = parser.parse_args()
    try:
        manifest = json.loads(
            (args.repo / "agents/capabilities.json").read_text(), object_pairs_hook=unique_object
        )
