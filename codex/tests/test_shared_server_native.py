    ...
    class Fixture(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            item = {
                ...
                "status": "completed",
                "content": [
                    {"type": "output_text", "text": "Offline fixture completed.", "annotations": []}
                ],
            }
            send(
            ...
            send(
                ...
                output_index=0,
                content_index=0,
                delta="Offline fixture completed.",
            )
        ...
        try:
            with log_path.open("w") as log:
                server = subprocess.Popen(
                    [str(binary), "app-server", "--listen", "unix://"],
                    cwd=root,
                    env=env,
            ...
            for index in range(2):
                connection = await websockets.unix_connect(
                    str(socket_path),
                    uri="ws://localhost/",
                    compression=None,
                    open_timeout=8,
                ...
                initialized = await client.call(
                    "initialize", {"clientInfo": {"name": f"native-probe-{index}", "version": "1"}}
                )
            ...
            turn_id = (
                await first.call(
                    ...
                    {
                        "threadId": thread_id,
                        "input": [{"type": "text", "text": "Disposable local fixture request."}],
                    },
                )
            )["turn"]["id"]
            assert await asyncio.to_thread(request_seen.wait, 8), (
                "native server did not reach the local fixture"
            )
            pid_file = codex_home / "app-server-daemon/app-server.pid"
            pid_file.parent.mkdir(exist_ok=True)
            started = subprocess.run(
                ["ps", "-p", str(server.pid), "-o", "lstart="],
                env=env,
                check=True,
                capture_output=True,
                text=True,
            ).stdout.strip()
            skewed = datetime.datetime.strptime(
                started, "%a %b %d %H:%M:%S %Y"
            ) + datetime.timedelta(seconds=3)
            record = json.dumps(
                {"pid": server.pid, "processStartTime": skewed.strftime("%a %b %e %H:%M:%S %Y")}
            )
...
"""
            for index in range(6):
                ...
                result = subprocess.run(
                    [
                        ...
                        str(repo),
                        str(root),
                        "stale-absence" if index == 4 else "listening",
                    ],
                    cwd=root,
                    env=env,
                    capture_output=True,
                ...
                assert set((await second.call("thread/loaded/list", {}))["data"]) == {
                    thread_id,
                    other_id,
                }
                assert (await first.call("thread/read", {"threadId": thread_id}))["thread"][
                    "status"
                ]["type"] == "active"
            ...
            assert (await first.call("thread/read", {"threadId": thread_id}))["thread"]["status"][
                "type"
            ] == "idle"
            assert set((await second.call("thread/loaded/list", {}))["data"]) == {
                thread_id,
            ...
            print(
                "PASS: two native clients retained both threads through six launcher checks; "
            ...
            try:
                await asyncio.gather(
                    *(client.connection.close() for client in clients), return_exceptions=True
                )
            finally:
