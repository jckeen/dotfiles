...
def read_frontmatter(path):
    ...
    (`key: >-` etc.) joined by single spaces. Two YAML forms this reader
    cannot represent fail loudly instead of silently mangling (issue #235):
    a blank line inside a block scalar (true YAML keeps the paragraph after
    it; this reader would drop it), and quoted scalar values (the quotes and
    backslash escapes would leak into the output as literal content)."""
