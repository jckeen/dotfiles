...
### What changed
- **`setup.sh`** — Added a self-healing block (after the `cd ~/dev` patch) that ensures `~/.bash_profile` sources `~/.bashrc`. If `.bash_profile` exists but doesn't already source `.bashrc`, the line is appended; if it doesn't exist, a minimal one is created. Idempotent (regex-guarded against re-adding).
