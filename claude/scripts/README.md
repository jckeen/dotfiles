...
| `check-doc-refs.sh` | Fails when a tracked Markdown doc references a hook or skill *path* that is not on disk: `<Word>.hook.ts` / `<Word>.hook.sh` must exist under `claude/hooks/`, and a `(claude\|codex\|agents\|antigravity)/skills/<name>/` form must be a real directory — and when a relative Markdown link (`[text](path)`) in a tracked doc resolves to nothing on disk, which is why an otherwise unrelated documentation edit can trip this step; external URLs, in-page anchors, and placeholder targets are skipped, code blocks and inline code are stripped first, and the docs allowlisted in the script (`CHANGELOG.md`, `SECURITY_FINDINGS_*.md`, `Plans/`, `docs/adr/`) may name removed files on purpose and are exempt from every check. Scope is otherwise deliberately narrow — a bare slash-command like `/clear` is not treated as a skill path, since many are Claude built-ins rather than repo dirs. Runs in the `checks` shards, step `doc-refs (self-test + checker)`. Tests: `tests/doc-refs.test.sh` | Read-only | No |
