...
| `harvest-codex-comments.sh` | Turns the GitHub Codex bot's inline review comments on a PR into **one issue per PR** (#556), titled `Codex review of #<n>: <PR title>`: a checklist with one item per comment carrying its priority badge, `path:line`, link, and the full comment body (past GitHub's body-size limit the items drop the quoted text and keep the rest). Dedup reads one REST prefetch of every issue body (shared with the review gate via `gate-lib.sh`): a `codex-review-pr` marker finds the PR's issue in any state, and new comments are **appended to its body** — re-read just before the edit so ticked boxes survive, reopening it if closed — rather than filed again; a per-comment `codex-comment-id` marker keeps any comment, including one tracked by a legacy per-comment issue, from being added twice. Appending edits the body, not an issue comment, because the prefetch reads bodies only. Outdated and resolved comments are skipped (#159). Applies `codex-finding`, plus `instruction-surface` when a comment's path matches the Codex gate's self-review guard regex, copied verbatim into the script and compared by the tests (#557). Runs at PR close from `.github/workflows/harvest-codex-comments.yml` (merged PRs only, under `GITHUB_TOKEN`; #555), nightly from the cloud docs-steward routine as a backstop, and early from the `PreMergeCodexHarvest.hook.sh` hook; the markers make every pair of runs idempotent. `--dry-run` writes nothing. Always exits 0. Tests: `tests/harvest-codex-comments.test.py` (also covers the hook's `--auto` note) | Issue writes (create, or edit one body) | No |
