...
def check(root):
    contract = json.loads(
        (root / "agents/workflow-invariants.json").read_text(), object_pairs_hook=unique_object
    )
    ...
    errors += [
        skill + ": no shared workflow in coverage contract"
        for skill in sorted(workflows.keys() - shared)
    ]
    for skill in sorted(shared & workflows.keys()):
        ...
        for name, invariant in invariants.items():
            if (
                ...
                or not isinstance(invariant.get("description"), str)
