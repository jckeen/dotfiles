...
class WorkflowInvariantTests(unittest.TestCase):
    def setUp(self):
        ...
        self.contract = dict(
            ...
            workflows={
                "simplify": {
                    "preserve-behavior": dict(
                        description="Simplification preserves behavior.",
                        claude=[r"preserve observable behavior"],
                        agents=[r"keep behavior unchanged"],
                    ),
                    "verification-required": dict(
                        description="Run the relevant verification after edits.",
                        claude=[r"run the project tests"],
                        agents=[r"run focused checks"],
                    ),
                }
            },
        )
        self.write_skill(
            "claude",
            "Use the native edit tool; preserve observable behavior.\nRun the project tests.\n",
        )
        self.write_skill(
            "agents",
            "Use available tools and keep behavior unchanged.\nRun focused checks.\nExtra runtime guidance is allowed.\n",
        )
    ...
    def test_unreadable_override_bundle_or_entrypoint_is_rejected(self):
        entrypoint = self.write_skill(
            "antigravity", "Keep behavior unchanged. Run focused checks.\n"
        )
    ...
    def test_stale_workflow_contract_fails(self):
        self.contract["workflows"]["retired"] = copy.deepcopy(
            self.contract["workflows"]["simplify"]
        )
