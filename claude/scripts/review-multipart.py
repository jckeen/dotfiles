...
def prepare(packet, directory):
    ...
    manifest = {
        "sha256": digest(raw),
        "catalogSha256": digest((directory / "catalog.json").read_bytes()),
        "parts": [],
    }
...
'''
    ...
    (directory / "ack-schema.json").write_text(
        json.dumps(
            {
                "type": "object",
                "properties": {"part": {"type": "integer"}, "sha256": {"type": "string"}},
...
def session_records(session):
    ...
    records = [
        strict_json(line) for line in Path(rows[0][0]).read_text(encoding="utf-8").splitlines()
    ]
    if (
        not records
        or records[0].get("type") != "session_meta"
        or records[0].get("payload", {}).get("id") != session
    ):
...
def check(directory, index, expected_session, expected_manifest):
    ...
    maximum = audit_history(
        session_records(sessions[0]), expected_inputs, strict_json(catalog_bytes)
    )
