...
class CapabilityParityTests(unittest.TestCase):
    def setUp(self):
        ...
        self.manifest = dict(
            version=1,
            capabilities={
                cap: {runtime: copy.deepcopy(row) for runtime in RUNTIMES} for cap in CAPABILITIES
            },
        )
    ...
    def test_command_probe_is_rejected(self):
        self.manifest["capabilities"]["review"]["claude"]["probe"] = [
            dict(kind="command", command="true")
        ]
        self.reject("review/claude")
    ...
    def test_claude_default_companion_import_is_present_without_local_alias(self):
        ...
        self.assertEqual(
            len(imports), 1, "Update this fixture if Claude changes its default import contract"
        )
        ...
        declared = json.loads((ROOT / "agents/capabilities.json").read_text())
        self.manifest["capabilities"]["private-memory"]["claude"] = declared["capabilities"][
            "private-memory"
        ]["claude"]
        result = self.run_check("--live-home", str(self.home))
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        row = next(
            row
            for row in json.loads(result.stdout)["live"]
