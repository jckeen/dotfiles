...
| **Antigravity permissions** | `antigravity/permissions.json` | Curated allow/ask/deny rules plus `toolPermission: proceed-in-sandbox` and `enableTerminalSandbox: true`. `setup.sh` merges it into the machine-local `~/.gemini/antigravity-cli/settings.json` via `claude/scripts/agy-apply-permissions.py` (never symlinked: that file also holds `trustedWorkspaces` and prompt-saved grants). `check-antigravity.sh` warns on drift; `agy-apply-permissions.py prune` resets accreted grants to the baseline after a backup |
