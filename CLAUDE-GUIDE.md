...
| `cx [project]` | Same launch ergonomics for Codex (runs `check-codex` instead); reuses a listening shared server when Remote Control is already enabled, starts a missing daemon with a deadline, and leaves existing sessions untouched on relay or ownership errors; preserves explicit endpoints and falls back locally if startup cannot be verified |
...
> **The primary dotfiles checkout stays on `main`.** `~/.bash_aliases`, hooks,
> skills and agents are symlinks into `~/dev/dotfiles`, so whatever branch is
> checked out there is the live shell config for every new shell. Do dotfiles
> branch work in a worktree (`wt-claude <name>` from `~/dev/dotfiles`); the
> `cc`/`cx`/`agy` preflight warns while the primary is off `main`.
