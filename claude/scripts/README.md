...
| `resolve-changelog.py` | The standard fix for a DIRTY `CHANGELOG.md` head (#561): after `git merge origin/main` stops on the changelog because two PRs each prepended a dated section, it keeps both sides' sections with origin's (merged first) on top and ours below, editing no entry text. It does not trust git's hunk boundaries: it rebuilds each side's whole file, splits both into `## ` sections, and treats the identical trailing run as shared history. It refuses, leaving the file untouched, unless the difference is purely a prepend: the same text above the first section, no heading in both sides' new sections or in a new section and the history (an edited existing section shows up this way), and, with diff3 markers, a base equal to the history. Merge, do not rebase (a rebase swaps the sides). `--check` reports without writing; re-running on a resolved file is a no-op. `CHANGELOG.md` itself holds the current quarter; older quarters are HISTORICAL files under `docs/changelog/`. Tests: `tests/resolve-changelog.test.py`, run in CI through `tests/doc-refs.test.sh` | Fix (edits `CHANGELOG.md`) | Yes — review with `git diff`, then stage it |
