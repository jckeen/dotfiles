...
- **Public-safe Codex parity** — the public-safe skill subset (review, simplify, fix-issue, commit-push-pr, handoff, changelog, repo-health, branch-hygiene), wired so `cx` mirrors `cc` on the shared workflows. Codex auth, sessions, sqlite state, and live `config.toml` stay local; only public-safe guidance and skills are shared.
...
│   │   └── tests/               # Fixture self-tests for the 7 gate checkers
